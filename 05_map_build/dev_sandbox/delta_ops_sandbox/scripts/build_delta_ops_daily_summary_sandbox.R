# ==== build_delta_ops_daily_summary_sandbox.R ===============================
#
# PURPOSE:
#   Sandbox parser for DWR's Delta Operations Daily Summary PDF.
#   This is NOT a BRIM patch yet. It is meant for local/GitHub-feed testing.
#
# OUTPUTS:
...
#
# DESIGN:
#   - Downloads the current daily PDF from DWR.
#   - Extracts text with pdftools::pdf_text().
#   - Parses the daily operations/accounting values BRIM wants to visualize.
#   - Joins values to static dashboard-point coordinates.
#   - Optionally snaps X2 to data/input/x2_river_km_lookup.csv if present.
#   - Writes a point GeoJSON suitable for a future Ops Live layer.
#
# HOW TO RUN LOCALLY FROM THE brim-live-data-feeds REPOSITORY ROOT:
#   Rscript scripts/build_delta_ops_daily_summary_sandbox.R
#
# REQUIRED/OPTIONAL INPUTS:
#   data/input/delta_ops_static_locations.csv       optional; default embedded
#   data/input/x2_river_km_lookup.csv               optional; river_km, lon, lat
#
# NOTES:
#   DWR labels this report PRELIMINARY DATA, SUBJECT TO REVISION WITHOUT NOTICE.
#   Treat these values as situational awareness / screening, not final data.
# ============================================================================
...
pt_values_from_section <- function(section_lines, labels, n_expected) {
  ...
  for (ln in section_lines) {
    ...
    # If the PDF extraction keeps a label and value on the same line, retain
    # only the content after the equals sign. If the equals sign is the whole
    # line or has no value after it, this becomes blank and is dropped.
    ...
    # Drop section headings that may be included at the start of a section.
  ...
  if (length(vals) < n_expected) {
    stop(
      "Could not parse enough values from section. Expected ", n_expected,
...
pt_default_static_locations <- function() {
  tibble::tribble(
    ~feature_key, ~display_name, ~feature_type, ~lat, ~lon, ~label_anchor, ~label_dx, ~label_dy, ~sort_order, ~placement_note,
    "cross_channel_gates", "Delta X-Channel Gates", "gate", 38.246551, -121.509171, "top", 0, 0, 10, "Represents Delta Cross Channel Gates percent of day open; operations/control point near the gates.",
    "jones_cvp_exports", "Jones / CVP exports", "export", 37.796724, -121.585311, "left", 0, 0, 20, "Represents CVP export pumping at Jones Pumping Plant / Tracy Pumping Plant.",
    "banks_swp_exports", "Banks / SWP exports", "export", 37.800748, -121.620824, "right", 0, 0, 30, "Represents SWP export-related value, especially Clifton Court Inflow / Banks Pumping Plant export context.",
    "delta_outflow_index", "Delta Outflow Index / NDOI", "outflow", 38.055198, -121.911904, "left", 0, 0, 40, "Calculated daily net freshwater outflow from the Delta past Chipps Island into Suisun Bay; not an instantaneous gage.",
    "percent_inflow_diverted", "Percent Inflow Diverted", "accounting", 38.055198, -121.911904, "right", 0, 0, 50, "Delta accounting ratio showing percent of Delta inflow diverted by south Delta exports.",
    "omr_index", "Old and Middle River Flow / OMR Index", "flow_index", 37.9560963, -121.5505, "bottom", 0, 0, 60, "Combined south Delta reverse-flow index based on tidally filtered Old River and Middle River flows; representative placement.",
    "sacramento_freeport", "Sacramento River input / Freeport", "inflow", 38.455664, -121.50162, "left", 0, 0, 70, "Sacramento River inflow contribution to Delta accounting at the Freeport boundary concept.",
    "san_joaquin_vernalis", "San Joaquin River input / Vernalis", "inflow", 37.67601249, -121.2662907, "right", 0, 0, 80, "San Joaquin River inflow contribution to Delta accounting at the Vernalis boundary concept.",
    "total_delta_inflow", "Total Delta Inflow", "inflow_total", 38.0658382, -121.3839537, "top", 0, 0, 90, "Combined Delta inflow accounting value; conceptual marker not a single physical gage.",
    "delta_conditions", "Delta Conditions", "status", 38.0346435, -121.5488411, "top", 0, 0, 100, "Whole-Delta operations status flag; conceptual central-Delta placement.",
    "controlling_factors", "Controlling Factor(s)", "status", 37.803613, -121.594220, "bottom", 0, 0, 110, "Operational/regulatory factor limiting or guiding south Delta exports; conceptual placement between Jones and Banks.",
    "san_luis_reservoir", "San Luis Reservoir storage / SWP-CVP storage", "storage_split", 37.054159, -121.108983, "top", 0, 0, 120, "San Luis Reservoir storage relevant to SWP/CVP operations; CVP component calculated as Total minus SWP when available."
  )
}
...
pt_read_locations <- function(path) {
  if (file.exists(path)) {
    ...
    # Coerce optional label offset columns for later bind_rows() with X2 feature.
  ...
  pt_default_static_locations()
}

pt_geojson_from_df <- function(df) {
  ...
  list(
    ...
    name = "BRIM Delta Ops Daily Summary",
    features = features
  )
}
...
if (length(lines) < 20) {
  stop("PDF text extraction returned too few lines; cannot parse Delta Ops summary.")
}

# ---- 5. Parse report date and section values --------------------------------
...
scheduled_vals <- pt_values_from_section(
  pt_section(lines, "SCHEDULED EXPORTS FOR TODAY", c("ESTIMATED DELTA HYDROLOGY")),
  scheduled_labels,
  2L
)

hydro_vals <- pt_values_from_section(
  pt_section(lines, "ESTIMATED DELTA HYDROLOGY", c("DELTA OPERATIONS")),
  hydro_labels,
  3L
)

ops_vals <- pt_values_from_section(
  pt_section(lines, "DELTA OPERATIONS", c("RESERVOIR STORAGES")),
  ops_labels,
  7L
)

storage_vals <- pt_values_from_section(
  pt_section(lines, "RESERVOIR STORAGES", c("RESERVOIR RELEASES")),
  storage_labels,
  5L
)
...
vals <- c(
  ...
  pt_make_named(ops_vals, c(
    "delta_conditions",
    "cross_channel_gates_percent_open",
    "outflow_index",
    "percent_inflow_diverted",
    "x2_position_yesterday",
    "controlling_factors",
    "omr_index_daily_value"
  )),
...
# ---- 6. Join to static locations and build dashboard feature rows -----------
...
metric_rows <- tibble::tibble(
  ...
  metric_name = c(
    ...
    "Sacramento River input", "San Joaquin River input", "Total Delta Inflow",
  ...
  value_raw = c(
    ...
    vals$total_delta_inflow,
    vals$delta_conditions,
    vals$controlling_factors,
    paste0(
      "Total ", pt_format_taf(vals$san_luis_total_storage),
      " | SWP ", pt_format_taf(vals$san_luis_swp_share),
      " | CVP ", if (!is.na(san_luis_cvp_taf)) paste0(format(round(san_luis_cvp_taf), big.mark = ",", scientific = FALSE), " TAF") else "NA"
    )
  ),
  value_numeric = c(
  ...
  label_text = c(
    ...
    paste0("Diverted: ", pt_safe_value(vals$percent_inflow_diverted)),
    paste0("OMR: ", pt_format_cfs(vals$omr_index_daily_value)),
    paste0("Sac River: ", pt_format_cfs(vals$sacramento_river)),
    paste0("SJ River: ", pt_format_cfs(vals$san_joaquin_river)),
    paste0("Total inflow: ", pt_format_cfs(vals$total_delta_inflow)),
    paste0("Delta: ", pt_safe_value(vals$delta_conditions)),
    paste0("Control: ", pt_safe_value(vals$controlling_factors)),
    paste0(
      "San Luis: Total ", if (!is.na(san_luis_total_taf)) format(round(san_luis_total_taf), big.mark = ",", scientific = FALSE) else "NA",
      " | SWP ", if (!is.na(san_luis_swp_taf)) format(round(san_luis_swp_taf), big.mark = ",", scientific = FALSE) else "NA",
      " | CVP ", if (!is.na(san_luis_cvp_taf)) format(round(san_luis_cvp_taf), big.mark = ",", scientific = FALSE) else "NA",
      " TAF"
    )
  ),
  symbol_class = c("gate", "export_cvp", "export_swp", "outflow", "diversion_percent", "omr", "inflow", "inflow", "inflow_total", "status", "control", "storage_split")
)
...
# Optional X2 current position feature from a river-km lookup table.
...
if (file.exists(x2_lookup_csv) && !is.na(x2_km)) {
  ...
  if (length(missing_x2) > 0) {
    warning("X2 lookup exists but is missing required column(s): ", paste(missing_x2, collapse = ", "))
  } else {
    x2 <- x2 |>
      dplyr::mutate(
...
# ---- 7. Validate and write outputs -----------------------------------------

required_feature_keys <- metric_rows$feature_key
missing_features <- setdiff(required_feature_keys, features$feature_key)
if (length(missing_features) > 0) {
  stop("Missing expected dashboard features after location join: ", paste(missing_features, collapse = ", "))
}

if (nrow(features) < 10) {
  stop("Too few Delta Ops dashboard features to publish: ", nrow(features))
}
...
summary <- list(
  ...
  preliminary_notice = "PRELIMINARY DATA; SUBJECT TO REVISION WITHOUT NOTICE",
  parser_note = "Sandbox parser. Promote to live feed only after repeated QA against DWR PDF formatting."
)
...
message("  DCC gates: ", vals$cross_channel_gates_percent_open)
