# scripts/preview_delta_ops_snapshot_v7.R
...
# Combine Delta status, control, and diverted percent into one compact dashboard label.
...
if (length(i_status) == 1) {
  ...
  control_txt <- if (length(i_control) == 1) sub("^Control:\\s*", "", pts$pretty_label[i_control]) else "NA"
...
pts$show_point <- !pts$feature_key %in% c("delta_conditions", "controlling_factors", "percent_inflow_diverted")
pts$show_label <- !pts$feature_key %in% c("controlling_factors", "percent_inflow_diverted")
pts$point_color <- point_color_for(pts$feature_key)
...
out_html <- "docs/data/delta_ops_snapshot_preview_v7.html"

htmlwidgets::saveWidget(
  m,
  out_html,
  selfcontained = TRUE,
  title = "BRIM Delta Ops Snapshot Preview v7"
)
