# scripts/preview_delta_ops_snapshot_v10.R
...
label_dir_for <- function(key) {
  dplyr::case_when(
    ...
    key == "cross_channel_gates" ~ "right",
    key == "delta_outflow_index" ~ "left",
    key == "omr_index" ~ "bottom",
    key == "x2_position_current" ~ "right",
    key == "jones_cvp_exports" ~ "right",
...
pretty_label <- function(key, lab, raw) {
  ...
  if (key == "controlling_factors") return(paste0("Export control: ", gsub("^Control:\\s*", "", lab)))
...
if (length(i_status) == 1) {
  ...
  control_txt <- if (length(i_control) == 1) sub("^Export control:\\s*", "", pts$pretty_label[i_control]) else "NA"
  diverted_txt <- if (length(i_divert) == 1) sub("^Diverted:\\s*", "", pts$pretty_label[i_divert]) else "NA"
  pts$pretty_label[i_status] <- paste0(
    "Delta: ", status_txt,
    "<br>Export control: ", control_txt,
...
m <- leaflet::leaflet(
  ...
  # Default preview view. Tweak lng/lat/zoom here if you want to nudge the opening map frame.
  leaflet::setView(
    lng = -121.66,
    lat = 38.30,
    zoom = 10.3
...
# Labels are added separately as label-only markers to avoid Leaflet placing HTML labels at map origin.
m <- m |>
  leaflet::addCircleMarkers(
    data = draw_pts,
    lng = ~lon, lat = ~lat,
...
# Attach text labels to the actual point locations. No auxiliary label points.
for (i in seq_len(nrow(draw_pts))) {
  row <- draw_pts[i, , drop = FALSE]
  off <- row$label_offset
...
if (nrow(status_pts) == 1) {
  m <- leaflet::addLabelOnlyMarkers(
    ...
    data = status_pts,
    lng = ~lon, lat = ~lat,
    label = htmltools::HTML(status_pts$label_html),
    labelOptions = leaflet::labelOptions(
      noHide = TRUE,
      textOnly = TRUE,
      direction = "top",
  ...
  setTimeout(function() {
    ...
    if (btn) {
      btn.addEventListener('click', function() {
        // Match this to the R leaflet::setView() default above when tweaking the zoom button.
        map.setView([38.30, -121.66], 10.3);
      });
    }
  }, 100);
}
...
out_html <- "docs/data/delta_ops_snapshot_preview_v10.html"

htmlwidgets::saveWidget(
  m,
  out_html,
  selfcontained = TRUE,
  title = "BRIM Delta Ops Snapshot Preview v10"
)
