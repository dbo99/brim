...
- Ops compact BRIM-M/BRIM-E badges, paired agency subtitles, and shared row presentation contracts: `/qa/test_ops_live_legends_and_scan_source.js`; actual authored delivery projection, Guide parity, six conservative radar/QPE classifications and negative fixtures: `/qa/test_guide_ops_delivery_parity.R`. The sole relationship-registry authority, curated-collection provenance, separate prepared-feed subset, descriptive metadata, startup framing and deferred source/status review are documented in `/08_docs/BRIM_DEVELOPMENT_ARCHITECTURE.md`.
