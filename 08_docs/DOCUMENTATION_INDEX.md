...
- BRIM Guide architecture and onboarding boundary:
  `/08_docs/BRIM_DEVELOPMENT_ARCHITECTURE.md` under **BRIM Guide foundation** (accepted V4.1 shell and typography roles, complete 272-item post-basemap A–Z layer/tool browse, and the one-controller 228-record Resource Explorer with exact `In BRIM map` 28 / `Beyond the map` 200 / `All Resources` 228 primary views derived only from reviewed Resource map representation; Resource-specific gateway search routing into `All Resources` without query/focus loss; restrained direct/selected/not-mapped statements; delivery and exact multiple-source lists as secondary detail; eleven named agency choices plus County, Nonprofits / NGOs, Other, Private, and Regional/local in alphabetical displayed/DOM order across three compact always-open Federal / State / Provider types fieldsets, with adjacent State/type groups that stack as space requires, explicit family mappings, counts used only as information and preserved removable hidden-family/exact-provider chips, comprehensive name-based search and compact high-value facets; a stationary outer Guide; independent desktop filter-body, middle-results, and right-detail native scroll owners, with a reserved More filters/Reset action row, a bounded narrow filter-body scroller, and reachable expanded controls with selected-row promotion, detail-top reset, and result/focus restoration; narrow one-pane behavior; exact canonical official-Resource actions; weighted search; 84 source-backed rich Product records; 11 typed Quick Access destinations; seven maintained Methods; three verified Updates; and current-build Legacy Notes retirement)
- Documentation-only Resource intake decision ledger:
  ...
  separately verified; the relationship validator requires the referenced file to exist but the builder
  does not parse it as a runtime catalog, the C2 amendment records its exact combined approval, while human visual
...
- BRIM Guide sole Product–Resource relationship registry:
  `/00_config/guide_product_resource_relationships.json` (schema version 2 with exactly one record for every one of 272 compiled Products and every one of 233 canonical Resources; sole Product-link and Resource map-representation authority; the full Resource set resolves to 3 direct, 25 selected-products, 200 not-currently-mapped, and 5 not-yet-reviewed records while the 228 published Resources retain the public 3/25/200 split; R16B preserves the crosswalk-proven SPK `source_reference` for `ops_cdec_reservoir_storage` and its existing CDEC relationship, adds exact selected-product relationships for Drought.gov California, the two CoCoRaHS Products, and the two 2024 Integrated Report geometry Products, removes two unsupported provider-similarity links from California Water Watch, and preserves the two existing USBR links at the historical 90-link R16B gate; subsequent CNRFC, WPC ERO, and exact BLM source-reference decisions reached 121 links across 93 Products at the historical C2 gate; current accepted satellite membership has 120 Product resource-link entries across 92 Products and 28 Resources; delivery remains secondary; temporary R12A objects and adapter are absent; ordinary relationship additions remain declarative data-only changes)
- BRIM Guide canonical authored Resource metadata registry:
  `/00_config/guide_resources.json` (one ordered schema-v3 dataset with 233 canonical Resources: 228 published/browser-visible and five staged; R16B applies the balanced 13-addition / 7-merge / 123-access-point USACE/USBR model, retaining the six merged Sacramento report identities and the merged LTO decision under truthful parent labels and aliases; `scc` is Success Dam & Lake, Water Control Manuals remains held and absent, and the stable statewide Integrated Report parent now uses the evergreen Water Boards assessment-program action plus accurate 2024 and 2026 cycle actions; Resource indexing/matching semantics are unchanged while the C-gateway input routes into `All Resources`; HTTPS remains the external-URL default, with only three exact Resource-ID/full-URL HTTP exceptions for TID WISKI, Kings River Water Association, and Orange County Hydrology, all user-initiated links rather than runtime services; publication projection removes staged records before public relationships, search, counts, facets, adaptation, and embedding, preserving the 28/200/228 public views; `resource_nasa_giovanni`, `resource_usgs_earthexplorer`, and `resource_usgs_water_data_apis` remain held for subject review, while `resource_nrcs_web_soil_survey` and `resource_usda_cropland_data_layer` remain held for taxonomy resolution; controlled Resource Type, temporal character, and geographic scope are required descriptive metadata; granularity remains editorial-only; Product relationships remain solely authored in `guide_product_resource_relationships.json`, and runtime authority remains elsewhere)
...
- BRIM Guide Information Type compiler, classification, disjunctive badge, and facet-rendering test: `/qa/test_guide_information_types.js`; classification policy is maintained in `/DATA.md`.
- Ops compact BRIM-M/BRIM-E badges, paired agency subtitles, and shared row presentation contracts: `/qa/test_ops_live_legends_and_scan_source.js`; actual authored delivery projection, Guide parity, six QPE Enhanced classifications and two provider-hosted radar exclusions and negative fixtures: `/qa/test_guide_ops_delivery_parity.R`. The sole relationship-registry authority, curated-collection provenance, separate prepared-feed subset, descriptive metadata, startup framing and deferred source/status review are documented in `/08_docs/BRIM_DEVELOPMENT_ARCHITECTURE.md`.
- Historical offline GIBS and WPC/CPC QA fixture identities, provenance, roles and ordinary source/offline commands: `/DATA.md` under **Source/offline QA and historical provider fixtures**. The 120 Product resource-link entries are distinct from 233 Resource records and 1313 delivery-evidence references across 58 repository paths.
- Installed-Leaflet radar contract QA: `/qa/test_ops_radar_leaflet_contract.js`, with an explicit installed Leaflet JS argument; three Evented/WMS/callback checks, modeled geometry/transport, no browser/provider requests. Existing Springs installed-library QA: `/qa/test_springs_interaction_ownership.js`. Missing/invalid library input fails; the library is not vendored.
- Accepted-work checkpoint boundary and remaining A/B/C program obligations: `/08_docs/BRIM_DEVELOPMENT_ARCHITECTURE.md` under **Accepted-work checkpoint and remaining program**. Retained native/build evidence needs external inputs and does not become ordinary repository QA.
