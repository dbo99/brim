...
- BRIM Guide sole Product–Resource relationship registry:
  `/00_config/guide_product_resource_relationships.json` (schema version 2 with exactly one record for every one of 270 compiled Products and every one of 211 canonical Resources; sole Product-link and Resource map-representation authority; the full Resource set resolves to 3 direct, 23 selected-products, 180 not-currently-mapped, and 5 not-yet-reviewed records while the 206 published Resources retain the public 3/23/180 split; R16B preserves the crosswalk-proven SPK `source_reference` for `ops_cdec_reservoir_storage` and its existing CDEC relationship, adds exact selected-product relationships for Drought.gov California, the two CoCoRaHS Products, and the two 2024 Integrated Report geometry Products, removes two unsupported provider-similarity links from California Water Watch, and preserves the two existing USBR links for a total of 90 canonical links; delivery remains secondary; temporary R12A objects and adapter are absent; ordinary relationship additions remain declarative data-only changes)
- BRIM Guide canonical authored Resource metadata registry:
  `/00_config/guide_resources.json` (one ordered schema-v3 dataset with 211 canonical Resources: 206 published/browser-visible and five staged; R16B applies the balanced 13-addition / 7-merge / 123-access-point USACE/USBR model, retaining the six merged Sacramento report identities and the merged LTO decision under truthful parent labels and aliases; `scc` is Success Dam & Lake, Water Control Manuals remains held and absent, and the stable statewide Integrated Report parent now uses the evergreen Water Boards assessment-program action plus accurate 2024 and 2026 cycle actions; Resource indexing/matching semantics are unchanged while the C-gateway input routes into `All Resources`; HTTPS remains the external-URL default, with only three exact Resource-ID/full-URL HTTP exceptions for TID WISKI, Kings River Water Association, and Orange County Hydrology, all user-initiated links rather than runtime services; publication projection removes staged records before public relationships, search, counts, facets, adaptation, and embedding, preserving the 26/180/206 public views; `resource_nasa_giovanni`, `resource_usgs_earthexplorer`, and `resource_usgs_water_data_apis` remain held for subject review, while `resource_nrcs_web_soil_survey` and `resource_usda_cropland_data_layer` remain held for taxonomy resolution; controlled Resource Type, temporal character, and geographic scope are required descriptive metadata; granularity remains editorial-only; Product relationships remain solely authored in `guide_product_resource_relationships.json`, and runtime authority remains elsewhere)
