...
- BRIM Guide sole Product–Resource relationship registry:
  `/00_config/guide_product_resource_relationships.json` (one schema-version-1 record for every compiled Product; delivery classification is independent of reviewed/not-yet-reviewed Resource coverage and zero-or-more exact canonical Resource links; ordinary relationship additions are declarative data-only changes; the compiler-owned temporary R12A public adapter is noncanonical and scheduled for deletion in R12B)
