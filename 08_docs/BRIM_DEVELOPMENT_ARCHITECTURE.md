...
The accepted V4 shell uses a dark contour outer field around one large warm off-white surface, a compact fixed left rail, and a search utility band confined to the main column. The rail owns Home identity, compact A Explore / B Methods & Guides / C Resources / D Updates navigation, one bounded plain-text Quick Access list, About / Contact, and lower DOI/BLM marks. The main Explore view uses a compact two-column identity introduction, scope note, dense browse facets, and the complete profile-projected layer/tool inventory sorted case-insensitively by display name with stable ID as the tie-breaker. The A–Z inventory uses the shared compact result renderer in one bounded scroll region so its first rows and browse facets remain visible on desktop. The shell has no masthead, footer, card grid, pill navigation, pagination, virtualization, or duplicate responsive implementation. Intermediate layouts retain a reduced rail; the mobile layout becomes one full-screen surface with one upper-right close control, search below the top bar, compact horizontal A–D navigation, the same Quick Access list, and no footer or horizontal overflow.

V4 behavior includes Home reset, Escape/close/focus restoration, exact-path detail headed `Find in layer list`, verified stable-ID Quick Access, and controlled browse by primary subject and visible `Information Type`. User-facing result types are Layer, Tool, Method, Resource, and Update while the internal Product umbrella remains unchanged. Search is deterministic and gives precedence to exact titles and reviewed aliases before paths, subject/mode/family, provider/program, concise source-supported summaries, related Resource titles, and conservative typo recovery. Runtime IDs, raw URLs, controller variables, and DOM text are not semantic search inputs.

The curated Guide I1 content floor enriches the three Quick Access destinations while preserving automatic basic coverage for all 280 included Products. `HUC8 – PRISM/BCMv8` describes current display modes, filters, exact-weight PRISM/BCMv8 processing, level-specific interpretation, 200 m shared-boundary display generalization, period/provenance, and role-labeled official sources. `Groundwater Basins – Bulletin 118` describes its three display modes, 515-record search/filter behavior, exact code-based SGMA 2019 join, 20 m display generalization, interpretation limits, and official sources. `Fire Perimeters` is the explicit three-layer collection `EXT070`, `EXT072`, and `EXT074`: CAL FIRE recent large-fire, CAL FIRE full historical, and NIFC current operational wildfire/complex perimeters remain distinct. `SCAN Soil Moisture` is a Guide-only display title for stable ID `ops_scan_soil_moisture`; the runtime layer label and exact map path are unchanged, and the Guide subject is explicitly `Soil Moisture`. Structured sections and relationship roles are compiled in R and rendered generically; the browser JS contains no record-ID-specific content branch.
