...
- the Guide compiler derives basemap identities from `pt_base_groups()` only to prove their explicit profile exclusion; basemap records are not embedded in Guide Products, while the small current Tools set derives from its existing visible controls.
