...
The accepted V4 shell uses a dark contour outer field around one large warm off-white surface, a compact fixed left rail, and a search utility band confined to the main column. The rail owns Home identity, compact A Explore / B Methods & Guides / C Resources / D Updates navigation, one plain-text Quick Access list, About / Contact, and lower DOI/BLM marks. The main Explore view uses a compact two-column identity introduction, scope note, dense browse facets, and initial Product rows without a masthead, footer, card grid, pill navigation, or duplicate responsive shell. Intermediate layouts retain a reduced rail; the mobile layout becomes one full-screen surface with one upper-right close control, search below the top bar, compact horizontal A–D navigation, the same Quick Access list, and no footer or horizontal overflow.

V4 behavior includes Home reset, Escape/close/focus restoration, exact-path Product detail headed `Find in layer list`, verified stable-ID Quick Access, and controlled browse by primary subject and data/guidance mode. Search is deterministic and gives precedence to exact titles and reviewed aliases before paths, subject/mode/family, provider/program, concise source-supported summaries, related Resource titles, and conservative typo recovery. Runtime IDs, raw URLs, controller variables, and DOM text are not semantic search inputs.

The curated Guide I1 content floor enriches the three Quick Access destinations while preserving automatic basic coverage for all 280 included Products. `HUC8 – PRISM/BCMv8` describes current display modes, filters, exact-weight PRISM/BCMv8 processing, level-specific interpretation, 200 m shared-boundary display generalization, period/provenance, and role-labeled official sources. `Groundwater Basins – Bulletin 118` describes its three display modes, 515-record search/filter behavior, exact code-based SGMA 2019 join, 20 m display generalization, interpretation limits, and official sources. `Fire Perimeters` remains an explicit two-Product collection (`EXT070` and `EXT074`) so CAL FIRE recent large-fire context is not conflated with NIFC current operational wildfire/complex perimeters. Structured sections and relationship roles are compiled in R and rendered generically; the browser JS contains no Product-ID-specific content branch.

The initial maintained Methods set contains exactly seven entries: How BRIM Works; Display Geometry & Generalization; SCAN Soil-Moisture Statistical Context; Snow-Pillow SWE Statistical Context; USGS Groundwater History Summaries; BRIM Live Update Timing; and BRIM Under the Hood. The Display Geometry Method compiles the current public disclosure rows from the maintained polygon-generalization registry. The initial Updates set contains three reverse-chronological, repository-verified entries: Read-only Layer Explorer added (2026-08-24), NBM accumulated QPF forecast windows added (2026-08-20), and NBM legend links simplified (2026-08-19). Resource records remain source links, not runtime data dependencies; the HUC8 record points to the official PRISM normals page and the USGS ScienceBase BCMv8 catalog item.
...
Guide I1 is read-only with respect to map state. Product detail reports the exact BRIM path, but `show_on_map` and `configure_on_map` are deferred to Guide I2; Guide metadata contains no layer/controller callbacks. Legacy Notes is absent from current builds: there is no toolbar entry, Guide destination, search record, embedded payload, hidden renderer, modal, or handler. Historical Notes content remains available only through older HTML artifacts or Git history and is not migrated into current Methods content.
