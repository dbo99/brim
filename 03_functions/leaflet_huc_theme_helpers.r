...
##
## PURPOSE:
##   Build and inject the HUC thematic-fill dropdown/legend controls.
##
## DESIGN:
...
pt_huc_legend_one <- function(df, huc_layer, theme_key, title, fill_col, label_col, bin_col, note = "") {

  if (!all(c(fill_col, label_col, bin_col) %in% names(df))) {
    return(list(
      title = paste(toupper(huc_layer), title),
      rows = list(),
      note = "No legend values found. Rebuild the core cache."
    ))
  }
  ...
  huc_theme_legends_nested <- purrr::imap(huc_all, function(x, nm) {
    ...
    list(
      ppt_in = pt_huc_legend_one(
        ...
        bin_col = "ppt_in_bin",
        note = "Blue scale; colors scaled within this HUC level."
      ),
      ppt_kaf = pt_huc_legend_one(
        ...
        bin_col = "ppt_kaf_bin",
        note = "Blue scale; volume is strongly affected by HUC area. Colors scaled within this HUC level."
      ),
      rech_in = pt_huc_legend_one(
        ...
        bin_col = "rech_in_bin",
        note = "Green scale; colors scaled within this HUC level."
      ),
      rech_kaf = pt_huc_legend_one(
        ...
        note = "Green scale; volume is strongly affected by HUC area. Colors scaled within this HUC level."
      )
    )
  })
