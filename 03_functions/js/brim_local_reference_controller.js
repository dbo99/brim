function(el, x, data) {
  ...
  function installCss() {
    ...
    style.textContent =
      ...
      '.pt-lr-compact-note{margin-top:4px;padding-top:3px;border-top:1px solid rgba(82,72,45,.23);color:#5a5144;font-size:10px;line-height:1.25}.pt-lr-map-details>.pt-lr-compact-note{margin:0;padding:1px 4px 3px 29px;border-top:0;font-size:9.5px}' +
      ...
      '.pt-local-reference-card[data-pt-local-reference-layer="federal_wilderness"] .pt-lr-summary{margin:2px 0}.pt-local-reference-card[data-pt-local-reference-layer="federal_wilderness"] .pt-lr-map-details{margin:2px 0;padding:0;font-size:9.5px}' +
  ...
  function createLayerController(layerData) {
    ...
    function createCard() {
      ...
      control.onAdd = function() {
        ...
        var cautionText = cleanText(layerData.caution);
        var generalizationText = cleanText(layerData.generalization_disclosure);
        var combinedNoteText = [cautionText, generalizationText].filter(Boolean).join(' ');
        var noteClass = generalizationText ? 'pt-lr-compact-note' : 'pt-lr-caution';
        var cautionHtml = federalData ? '' : (acecData || nationalMonuments || desertNclData) ?
          ...
          '<div class="' + noteClass + '">' + escapeHtml(combinedNoteText) + '</div></details>' :
          '<div class="' + noteClass + '">' + escapeHtml(combinedNoteText) + '</div>';
        ...
        var mapDetailsHtml = layerData.show_component_count ?
          (federalData ?
            ...
            '<div class="' + noteClass + '"><span class="pt-lr-map-details-text"></span> ' +
            escapeHtml(combinedNoteText) + '</div></details>' :
